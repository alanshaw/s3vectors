...
Set S3VECTORS_FULL=1 to run derived-field checks over every dataset size.
"""
...
class TestCorpus(unittest.TestCase):
    ...
    def test_full_corpus_datagen(self):
        # Every non-slice dataset must materialize (slice bounds are corpus-
        # linted). Derived fields regenerate their dataset internally, so they
        # run only where the regenerated bytes are small — pure-Python CRC is
        # slow. S3VECTORS_FULL=1 removes both caps.
        full = os.environ.get("S3VECTORS_FULL") == "1"
        generate_cap = float("inf") if full else 8 * 1024 * 1024
        derived_cap = float("inf") if full else 64 * 1024
        for file in s3v.load_all():
            for v in file["vectors"]:
