...
fn full_corpus_datagen_pass() {
    // Every non-slice dataset must materialize (slice bounds are corpus-linted).
    // Derived fields regenerate their dataset internally, so exercise them only
    // where the regenerated bytes are small; the algorithms are size-independent
    // and pinned by check values above.
